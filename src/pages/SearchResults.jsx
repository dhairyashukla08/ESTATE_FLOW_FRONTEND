...
const EMPTY = {
  city: "", purpose: "Buy", category: [], bedrooms: [], furnishing: [],
  minPrice: "", maxPrice: "", minArea: "", maxArea: "", bathrooms: "", sort: "newest",
};
...
const handleSortChange = (e) => {
  ...
  const activeCount =
    localFilters.category.length + localFilters.bedrooms.length + localFilters.furnishing.length +
    ["city", "minPrice", "maxPrice", "minArea", "maxArea", "bathrooms"].filter((k) => localFilters[k]).length;
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
          <div>
            <h1 className="text-4xl font-black mb-4 tracking-tight">
              {localFilters.purpose === "Rent" ? "Properties for Rent" : "Properties for Sale"}
            </h1>
            <p className="text-gray-500 mt-2">
              Found <span className="font-semibold text-gray-900">{properties.length}</span> verified listings in
              <span className="font-medium text-gray-900 capitalize"> {localFilters.city || "All Cities"}</span>
            </p>
          </div>
        </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 space-y-6 sticky top-24 max-h-[calc(100vh-8rem)] overflow-y-auto">
              <div className="flex justify-between items-center border-b pb-4">
                <h3 className="text-lg font-bold">
                  Filters {activeCount > 0 && <span className="text-xs text-gray-400">({activeCount})</span>}
                </h3>
                <button onClick={handleClear} className="text-xs text-gray-400 hover:text-black font-bold uppercase transition">
                  Reset
                </button>
              </div>
              ...
              {/* City */}
              <div>
                <label className="block text-xs font-black uppercase text-gray-400 mb-2">Location</label>
                <input
                  type="text"
              ...
              {/* Property type (multi) */}
              <div>
                <label className="block text-xs font-black uppercase text-gray-400 mb-3">Property Type</label>
                <div className="space-y-2">
                  {CATEGORIES.map((cat) => (
                    <label key={cat} className="flex items-center gap-3 cursor-pointer text-sm font-medium text-gray-700 hover:text-black">
                      <input
                        type="checkbox"
              ...
              {/* BHK */}
              <div>
                <label className="block text-xs font-black uppercase text-gray-400 mb-3">Bedrooms</label>
              ...
              {/* Furnishing */}
              <div>
                <label className="block text-xs font-black uppercase text-gray-400 mb-3">Furnishing</label>
                <div className="space-y-2">
                  {FURNISHING.map((f) => (
                    <label key={f} className="flex items-center gap-3 cursor-pointer text-sm font-medium text-gray-700 hover:text-black">
                      <input
                        type="checkbox"
                        className="w-4 h-4 accent-black rounded"
              ...
              {/* Price */}
              <div>
                <label className="block text-xs font-black uppercase text-gray-400 mb-2">Price Range (₹)</label>
              ...
              {/* Area */}
              <div>
                <label className="block text-xs font-black uppercase text-gray-400 mb-2">Area (sq.ft)</label>
                <div className="flex items-center gap-2">
                  <input type="number" placeholder="Min" value={localFilters.minArea}
                    onChange={(e) => set({ minArea: e.target.value })}
                    className="w-full px-3 py-2 rounded-xl border border-gray-200 outline-none focus:border-black text-sm" />
                  <input type="number" placeholder="Max" value={localFilters.maxArea}
              ...
              {/* Bathrooms */}
              <div>
                <label className="block text-xs font-black uppercase text-gray-400 mb-2">Bathrooms (min)</label>
                <select
          ...
          <div className="flex-1">
            {loading ? (
              <div className="flex flex-col gap-6">
                {[1, 2, 3].map((n) => <div key={n} className="h-72 bg-gray-200 animate-pulse rounded-[32px] w-full" />)}
              </div>
            ) : properties.length > 0 ? (
              <div className="flex flex-col gap-6">
                {properties.map((property) => (
                  <PropertyCard key={property._id} property={property} layout="horizontal" matchedFilters={getResidentialTags(property, appliedFilters)} />
                ))}
                {properties.length > 3 && (
              ...
              <div className="bg-white rounded-[32px] p-20 text-center border border-dashed border-gray-200">
                ...
                <p className="text-gray-500 mt-2">Try widening your price or area range.</p>
                <button onClick={handleClear} className="mt-6 px-8 py-3 bg-black text-white rounded-xl font-bold">
                  Clear Filters
                </button>
              </div>
            )}
