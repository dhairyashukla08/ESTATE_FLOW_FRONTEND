...
const AddProperty = () => {
  ...
  const { fetchFeatured, fetchCategoryData, fetchProperties } =
    usePropertyContext();
  ...
  const [formData, setFormData] = useState({
    ...
    furnishedStatus: editData?.furnishedStatus || "Unfurnished",
  ...
  const handleTabChange = (tab) => {
    ...
    setFormData((prev) => ({
      ...prev,
      propertyType:
        tab === "Plot" ? "Residential" : tab === "Commercial" ? "Office" : "",
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const toastId = toast.loading(
      editData ? "Updating property..." : "Uploading property details...",
    );
    try {
      ...
      if (uploadedImageUrls.length === 0) {
        ...
        toast.update(toastId, {
          render: "Please upload at least one image",
          type: "error",
          isLoading: false,
          autoClose: 3000,
        });
    ...
    <div className="min-h-screen bg-gray-50 px-6 py-12">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm p-10">
        ...
        <h2 className="text-2xl font-semibold text-gray-900 mb-10">
          {editData ? `Edit ${activeTab}` : `List New ${activeTab}`}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {activeTab === "Residential" && formData.purpose === "Rent" && (
            <div className="grid md:grid-cols-2 gap-6">
              <div className="flex flex-col">
                <label className="text-sm text-gray-600 mb-2">Furnishing</label>
                <select
