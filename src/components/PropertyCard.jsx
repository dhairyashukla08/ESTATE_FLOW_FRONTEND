...
const PropertyCard = ({ property, layout = 'vertical' }) => {
  ...
  const displayArea = property.features?.areaSize || property.features?.carpetArea || property.features?.plotArea || "N/A";

  // 3. Agent/Seller Name (Handles populated 'seller' object or flat strings)
  const displayAgent = property.seller?.name || property.agentName || property.agent?.name || "Verified Agent";
  ...
  return (
    <div className="group bg-white rounded-[32px] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-100">
      <div className="relative h-64 overflow-hidden bg-gray-100">
        <img src={images[currentImageIndex]} alt={property.title} className="w-full h-full object-cover transition-opacity duration-500" />
        {images.length > 1 && (
          <div className="absolute inset-0 flex items-center justify-between px-4 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={prevImage} className="p-2 rounded-full bg-white/90 hover:bg-white shadow-lg text-black">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15 19l-7-7 7-7" /></svg>
            </button>
            <button onClick={nextImage} className="p-2 rounded-full bg-white/90 hover:bg-white shadow-lg text-black">
      ...
      <div className="p-6 flex flex-col">
        <div className="flex justify-between items-start">
          <h3 className="text-lg font-black text-gray-900 leading-tight">{property.title}</h3>
          <p className="text-lg font-black text-gray-900">₹{property.price?.toLocaleString("en-IN")}</p>
        </div>
        ...
        <div className="mt-6 pt-4 border-t border-gray-100 flex justify-between text-sm">
          ...
          <div>
            ...
            <p className="font-black text-gray-900 truncate w-24">{property.address?.area}</p>
          </div>
        </div>

        <div className="mt-6 flex gap-2">
          <a href={`tel:${contactNumber}`} className="flex-1 px-4 py-2.5 rounded-xl border-2 border-black text-black font-bold text-xs hover:bg-black hover:text-white transition-all text-center">
            Call Agent
          </a>
          <Link to={`/property/${property._id}`} className="flex-1 px-4 py-2.5 rounded-xl bg-black text-white font-bold text-xs hover:bg-gray-800 transition-all text-center">
            Full View
          </Link>
        </div>
      </div>
